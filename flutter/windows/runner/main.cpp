...
{
  HINSTANCE hInstance = LoadLibraryA("libdarkdesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libdarkdesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (!c_args)
  {
    ...
    // std::cout << "DarkDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"DarkDesk";
  FUNC_RUSTDESK_GET_APP_NAME get_darkdesk_app_name = (FUNC_RUSTDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_darkdesk_app_name");
  if (get_darkdesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_darkdesk_app_name(app_name_buffer, 512) == 0) {
