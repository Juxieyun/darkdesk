...
class PlatformFFI {
  ...
  DarkdeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libdarkdesk.so')
        : isLinux
            ? DynamicLibrary.open('libdarkdesk.so')
            : isWindows
                ? DynamicLibrary.open('libdarkdesk.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibdarkdesk.dylib") :
  ...
  void _startListenEvent(DarkdeskImpl darkdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = darkdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
