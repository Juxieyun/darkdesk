...
#define RUSTDESK_LIB_PATH "libdarkdesk.so"
...
bool flutter_rustdesk_core_main() {
   void* libdarkdesk = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   if (!libdarkdesk) {
      fprintf(stderr,"Failed to load \"libdarkdesk.so\"\n");
      char* error;
   ...
   auto core_main = (RustDeskCoreMain) dlsym(libdarkdesk,"rustdesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
