...
hbb_name = 'darkdesk' + ('.exe' if windows else '')
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/lib/darkdesk')
    system2('mkdir -p tmpdeb/etc/darkdesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/darkdesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/darkdesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/lib/darkdesk/')
    system2(
        'cp ../res/darkdesk.service tmpdeb/usr/share/darkdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/darkdesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/darkdesk.svg')
    system2(
        'cp ../res/darkdesk.desktop tmpdeb/usr/share/applications/darkdesk.desktop')
    system2(
        'cp ../res/darkdesk-link.desktop tmpdeb/usr/share/applications/darkdesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/darkdesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/darkdesk/')
    system2(
        'cp ../res/pam.d/darkdesk.debian tmpdeb/etc/pam.d/darkdesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/darkdesk/files/polkit && chmod a+x tmpdeb/usr/share/darkdesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file('usr/share/darkdesk/files/systemd/darkdesk.service')
    system2('dpkg-deb -b tmpdeb darkdesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('darkdesk.deb', '../darkdesk-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/darkdesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/lib/darkdesk/')
    system2(
        'cp ../res/darkdesk.service tmpdeb/usr/share/darkdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/darkdesk.png')
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libdarkdesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/libdarkdesk.dll"):
    ...
    os.chdir('../..')
    if os.path.exists('./darkdesk_portable.exe'):
        os.replace('./target/release/darkdesk-portable-packer.exe',
                   './darkdesk_portable.exe')
    else:
        os.rename('./target/release/darkdesk-portable-packer.exe',
                  './darkdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/darkdesk_portable.exe')
    os.rename('./darkdesk_portable.exe', f'./darkdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/darkdesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/darkdesk.exe')
        system2('mv target/release/darkdesk.exe target/release/DarkDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\darkdesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/DarkDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/darkdesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/darkdesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/darkdesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv darkdesk-%s-0-x86_64.pkg.tar.zst darkdesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./darkdesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/darkdesk-%s-0.x86_64.rpm ./darkdesk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall darkdesk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/darkdesk-%s-0.x86_64.rpm ./darkdesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall darkdesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/darkdesk*.deb ./flutter/darkdesk.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/DarkDesk.app/Contents/MacOS/darkdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/DarkDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/darkdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/DarkDesk.app/Contents/MacOS/darkdesk
    #rcodesign sign --p12-file ~/.p12/darkdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/DarkDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/darkdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/DarkDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/DarkDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/DarkDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "DarkDesk %s.dmg" "target/release/bundle/osx/DarkDesk.app"' % version)
                os.rename('DarkDesk %s.dmg' %
                          version, 'darkdesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/darkdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./darkdesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./darkdesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple darkdesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/DarkDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/darkdesk*.deb ./darkdesk.deb')
                system2('dpkg-deb -R darkdesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/darkdesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/darkdesk-link.desktop tmpdeb/usr/share/applications/darkdesk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/darkdesk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/darkdesk/')
                os.system('mkdir -p tmpdeb/etc/X11/darkdesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/darkdesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/darkdesk.debian tmpdeb/etc/pam.d/darkdesk')
                system2('strip tmpdeb/usr/bin/darkdesk')
                system2('mkdir -p tmpdeb/usr/lib/darkdesk')
                system2('mv tmpdeb/usr/bin/darkdesk tmpdeb/usr/lib/darkdesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/lib/darkdesk/')
                md5_file('usr/share/darkdesk/files/systemd/darkdesk.service')
                md5_file('etc/darkdesk/startwm.sh')
                md5_file('etc/X11/darkdesk/xorg.conf')
                md5_file('etc/pam.d/darkdesk')
                md5_file('usr/lib/darkdesk/libsciter-gtk.so')
                system2('dpkg-deb -b tmpdeb darkdesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('darkdesk.deb', 'darkdesk-%s.deb' % version)
